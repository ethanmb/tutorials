...
function writeRecipesToDatabase(recipes){
  ...
  db.serialize(function() {
     ...
     sqlString = "CREATE TABLE IF NOT EXISTS recipes (id INTEGER PRIMARY KEY, recipe_name TEXT, spices TEXT, contributor TEXT, description TEXT, category TEXT, source TEXT, rating TEXT, ingredients TEXT, directions TEXT)";
     ...
     */
     var stmt = db.prepare("INSERT INTO recipes (recipe_name,spices,contributor,description,source,category,rating,ingredients,directions) VALUES (?,?,?,?,?,?,?,?,?)");
     for (var i = 0; i < recipes.length; i++) {
   	    recipe = recipes[i];
        stmt.run(recipe.recipe_name, recipe.spices, recipe.contributor, recipe.description, recipe.source, recipe.category, recipe.rating, recipe.ingredients, recipe.directions);
     }
     stmt.finalize();
  
     db.each("SELECT id, recipe_name, spices contributor, description, source, category, rating, ingredients, directions FROM recipes", function(err, row) {
         console.log(row.id + ": " +
             		 row.recipe_name + " " +
					 row.spices);
     });  
